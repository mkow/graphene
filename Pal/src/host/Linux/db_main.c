...
noreturn void pal_linux_main(void* initial_rsp, void* fini_callback) {
    ...
    char* manifest = NULL;
    char* exec_uri = NULL; // TODO: remove
    if (first_process) {
    ...
    if (!exec_uri) {
        ret = toml_string_in(g_pal_state.manifest_root, "loader.entrypoint", &exec_uri);
        if (ret < 0) {
            INIT_FAIL_MANIFEST(PAL_ERROR_INVAL, "Cannot read or parse \'loader.entrypoint\'\n");
        }
    }
