...
struct proc_args {
    ...
    size_t       manifest_path_size; // manifest path will follow this struct on the pipe.
...
int sgx_create_process(const char* uri, size_t nargs, const char** args, int* stream_fd,
    ...
    proc_args.manifest_path_size = strlen(g_pal_enclave.manifest_path);
    ...
    ret = INLINE_SYSCALL(write, 3, fds[1], g_pal_enclave.manifest_path,
                         proc_args.manifest_path_size);
    if (IS_ERR(ret) || (size_t)ret != proc_args.manifest_path_size) {
        ret = IS_ERR(ret) ? ret : -EINTR;
        goto out;
    }
...
int sgx_init_child_process(int parent_pipe_fd, struct pal_sec* pal_sec, char** manifest_path_out,
                           char** manifest_out) {
    int ret;
    struct proc_args proc_args;
    ...
    manifest_path = malloc(proc_args.manifest_path_size + 1);
    if (!manifest_path) {
        ret = -ENOMEM;
        goto out;
    }
    ...
    bytes_read = INLINE_SYSCALL(read, 3, parent_pipe_fd, manifest_path,
                                proc_args.manifest_path_size);
    if (IS_ERR(bytes_read)) {
        ret = IS_ERR(bytes_read) ? bytes_read : -EINTR;
        goto out;
    }
    manifest_path[proc_args.manifest_path_size] = '\0';
