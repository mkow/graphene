...
static int initialize_enclave(struct pal_enclave* enclave, const char* manifest_to_measure) {
     ...
     * this choice are:
     *    - For PIE binaries it must cover code segment loaded at some hardcoded address
     *      (usually 0x400000).
     ...
     * any pages at 0). This was fixed in the newer SGX drivers, so we can always use this range.
     */
