...
static void set_debug_type (void)
{
    ...
    ret = get_config(g_pal_state.root_config, "loader.debug_type",
    ...
    if (!strcmp_static(cfgbuf, "inline")) {
        ...
        ret = get_config(g_pal_state.root_config, "loader.debug_file",
                         cfgbuf, sizeof(cfgbuf));
        if (ret <= 0)
...
noreturn void pal_main(
    ...
    if (!exec_handle && g_pal_state.root_config) {
        ret = get_config(g_pal_state.root_config, "loader.exec",
                         uri_buf, URI_MAX);
        if (ret > 0) {
            exec_uri = malloc_copy(uri_buf, ret + 1);
