...
def generate_measurement(enclave_base, attr, areas):
    ...
    def include_page(digest, addr, flags, content, measure):
        ...
        # print('XYZ adding page 0x%08x (0x%04x, %02x %02x %02x %02x)' % (addr - enclave_base, _hash(content), content[256+3], content[256+7], content[256+11], content[256+15]))
...
def main_sign(args):
    ...
    if manifest.get('sgx.static_address', None) is None:
        ...
        # non-PIE executable
        # TODO: this detection is completely broken, wtf
        manifest['sgx.static_address'] = '0'
